...
esp_err_t dgx_st7735_init(dgx_screen_t *scr, gpio_num_t rst, uint8_t color_bits, dgx_color_order_t cbo) {
    ...
    scr->frame_buffer = 0;
    scr->trans[0].tx_data[0] = ST7735_CASET;           //Column Address Set
    scr->trans[2].tx_data[0] = ST7735_RASET;           //Page address set
    scr->trans[4].tx_data[0] = ST7735_RAMWR;           //memory write
