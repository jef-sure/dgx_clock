...
static inline void* set_dc(gpio_num_t dc, uint8_t val, uint8_t need_free) {
    need_free = !!need_free;
    return (void*) ((val & 1) | (need_free << 1) | ((uint32_t) dc << 2));
}
...
void dgx_spi_sync_send_cmd(dgx_screen_t *scr, const uint8_t cmd) {
    ...
    t.user = set_dc(scr->dc, 0, 0);                //D/C needs to be set to 0
...
void dgx_spi_sync_send_data(dgx_screen_t *scr, const uint8_t *data, int len) {
    ...
    t.user = set_dc(scr->dc, 1, 0); //D/C needs to be set to 1
    ret = spi_device_transmit(scr->spi, &t);  //Transmit!
    assert(ret == ESP_OK);            //Should have had no issues.
}
...
static void dgx_spi_pre_transfer_callback(spi_transaction_t *t) {
    uint32_t dc = (uint32_t) t->user;
    gpio_set_level((gpio_num_t) (dc >> 2), dc & 1);
}
...
static void dgx_spi_init_transactions(dgx_screen_t *scr) {
    for (uint16_t i = 0; i < 5; i++) {
        if ((i & 1) == 0) {
            ...
            scr->trans[i].user = set_dc(scr->dc, 0, 0);                //D/C needs to be set to 0
        } else {
            //Odd transfers are data
            scr->trans[i].length = 8 * 4;
            scr->trans[i].user = set_dc(scr->dc, 1, 0);               //D/C needs to be set to 1
        }
...
esp_err_t dgx_spi_init(dgx_screen_t *scr, spi_host_device_t host_id, gpio_num_t mosi, gpio_num_t sclk, gpio_num_t cs,
    ...
    devcfg.queue_size = scr->max_transactions;
    devcfg.pre_cb = dgx_spi_pre_transfer_callback;
    esp_err_t ret;
...
//Attach the LCD to the SPI bus
    ret = spi_bus_add_device(host_id, &devcfg, &scr->spi);
    assert(ret == ESP_OK);
    if (ret != ESP_OK) return ret;
...
void dgx_spi_allocate_queue(dgx_screen_t *scr, uint16_t free_q) {
    ...
    while (scr->pending_transactions && scr->max_transactions - scr->pending_transactions < free_q) {
        ...
        if ((uint32_t) (rtrans->user) & 2) {
            free((void*) rtrans->tx_buffer);
            free(rtrans);
        }
    }
}
...
void dgx_spi_write_data(dgx_screen_t *scr, uint8_t *data, uint32_t lenbits) {
    ...
    scr->trans_data->user = set_dc(scr->dc, 1, 0);
...
void dgx_spi_fill_rectangle(dgx_screen_t *scr, int16_t x, int16_t y, int16_t w, int16_t h, uint32_t color) {
    ...
    if (chunk_len > scr->draw_buffer_len * 8) {
        ...
        fb -= fb & 7; // align chunk to 8 pixels
        chunk_len = fb * 8;
    } else {
